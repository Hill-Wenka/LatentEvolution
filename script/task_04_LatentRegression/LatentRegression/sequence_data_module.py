...
class SequenceDataModule(pl.LightningDataModule):
    ...
    def prepare_dataset(self):
        ...
        self.features = load_prediction_data['z']
    ...
    def prepare_predict_data(self, predict_data='valid', data_class='Protein', **kwargs):
        ...
        if isinstance(predict_data, str):
            ...
            predict_data = [data_class(self.predict_index[i], self.predict_index[i], seq, attributes={self.data_args.label: -1})
    ...
    def collate_fn(self, batch):
        batch_seqs = [protein.sequence for i, protein in enumerate(batch)]
        batch_features = torch.stack([self.features[int(protein.index)] for i, protein in enumerate(batch)])
        batch_ddG = torch.tensor([protein.attributes['ddG'] for i, protein in enumerate(batch)])
        batch_dS = torch.tensor([protein.attributes['dS'] for i, protein in enumerate(batch)])
        batch_ids, batch_seqs, batch_tokens = self.tokenizer.tokenize(batch_seqs)
        return batch_tokens, batch_features, batch_ddG, batch_dS
